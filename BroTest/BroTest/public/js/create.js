$(document).ready(function() {
  ...
  // In localhost:8080/create?bro_id=1, broId is 1
  ...
  $(createForm).on("submit", function handleFormSubmit(event) {
    ...
    // Otherwise run submitbro to create a whole new bro
    if (updating) {
      newbro.id = broId;
      updatebro(newbro);
    }
  ...
  function getbroData(id) {
    $.get("/api/bros/" + id, function(data) {
      if (data) {
        // If this bro exists, prefill our create forms with its data
